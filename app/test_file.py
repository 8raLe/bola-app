...
test_results.append(test_endpoint("GET", "orders/6", user3_access, 200, "bob", "403_First"))
test_results.append(test_endpoint("GET", "orders/6", user3_access, 200, "bob", "404_First"))
test_results.append(test_endpoint("GET", "orders/6", user3_access, 200, "bob", "Query_Focused"))
...
# test of acccess to user orders
...
# Test if user2 can modify an order by user3
test_results.append(test_endpoint("PUT", "orders/5", user2_access, 403, "john", "404 *", {"status": "Shipped"}))

# Test if user3 can modify their own order
test_results.append(test_endpoint("PUT", "orders/5", user3_access, 200, "bob", "404 *", {"status": "Shipped"}))

# Test if user2 can delete user3's order
test_results.append(test_endpoint("DELETE", "orders/8", user2_access, 404, "john", "404 *"))

# Test if user3 can delete his own order (should succeed)
test_results.append(test_endpoint("DELETE", "orders/8", user3_access, 200, "bob", "404 *"))

# Test HTTP method confusion, trying to bypass with different HTTP methods
test_results.append(test_endpoint("POST", "orders/6", user2_access, 405, "john"))

# Test parameter pollution, access via query paramaters instead of path
test_results.append(test_endpoint("GET", "orders?id=6", user2_access, 404, "john"))
            ...
            for pattern, patterns_data in username_data.items():
                ...
                if 403 in patterns_data:
                    ...
                    if resource_type == "users":
                        ...
                        valid_range = list(range(1, 7))  # 6 orders
